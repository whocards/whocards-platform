...
const context = (query: string): APIContext => ({url: new URL(`https://whocards.cc/api/print.pdf${query}`)}) as APIContext
...
describe('GET /api/print.pdf', () => {
  ...
  it('returns 400 for an unsupported language', async () => {
    const res = await GET(context('?deck=library&lang=he&preset=avery-5371'))
    expect(res.status).toBe(400)
    const body = await res.json()
