...
Date: 2026-07-05 · Status: research + recommendation, Slack POC spiked in PR #219, no production
code shipped · Issue: #179 · Epic: #201 ("WhoCards @ Work platform")
...
5. **A real correctness bug carried over from spike 0002's optimistic rendering plan:** it says "no
   ...
   an image. See §7.
6. **Spiked in PR #219:** a stateless `/whocards [deck] [language]` Slack slash command —
...
### Phase 0 — Stateless pilot (spiked in PR #219, §9)
...
- **Where it runs:** Phase 0 pilot → `apps/website` (`apps/website/src/pages/api/bots/slack/*`, PR
  #219's spike). Phase 1+ → `apps/app` if merged by then (`apps/app/src/routes/api/slack/*`, a
...
- **Secrets, env-gated, optional (never build-required — this is a pilot, not a shipped feature):**
  `SLACK_SIGNING_SECRET` (Phase 0, spiked in PR #219), `SLACK_CLIENT_ID`/`SLACK_CLIENT_SECRET`
  (Phase 1 OAuth), and per-workspace bot tokens (Phase 1+, stored in the DB, never in env/logs).
...
Added in PR #219, all env-gated and degrading cleanly when unconfigured:
...
app's **Signing Secret** into `SLACK_SIGNING_SECRET`. PR #219's Netlify deploy preview
(`apps/website` is git-integrated for previews) gives a public URL to point the Request URL
...
1. **Phase 1: OAuth install + persisted per-channel draw state** — the funnel hook that works for
   any workspace, not just a hardcoded pilot. Depends on the Phase 0 spike in PR #219 (still open).
