...
A minimal spike ships separately in PR #220: `/dev/tv-board` (dev-only, `apps/website`) — one deck
rendered in big-screen type next to a mock room code, no backend wiring. It's there to let you
eyeball the visual direction on an actual screen; it does not prove the pairing mechanism (§3),
which is unbuilt. See §6 and the screenshot in this doc.
...
The spike lives on PR #220, not in this docs-only PR — the screenshot below is what it produces.
...
To view it live: check out PR #220's branch, run `pnpm --filter website dev`, then open
`http://localhost:4321/dev/tv-board` (ideally cast/mirror that tab to an actual TV — the whole
point is judging 10-foot readability, which a laptop screen can't tell you).
